...
#此次只取小类图片，有重复的图片连接要去重
# demo12数据太多，分出此文件来取分页面的图片并保存，已成功
...
for i in pages:
    ...
    for p in range(1,i2+1):
        ...
        for yu in zyurl:
            ...
            #大类下每所专业院校的图片，此次只取一张，因为其他的6张都是重复的，到时候直接取随意一个院校的就可
            html4 = etree.HTML(c4)
...
yximage2 = set(yximage)
yximage3 = list(yximage2)
# print(yximage3)
# print(type(yximage3))

for j in range(len(yximage3)):         #遍历所有图片url，并保存到本地路径，放在文件夹里吧，想想办法
    print("%s" % (yximage3[j]))
    header = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"}
    ...
    filepath = './01'
    if not os.path.exists(filepath):  # 如果文件夹01不存在就创建
        os.mkdir(filepath)
    p2 = filepath + '/%s.jpg' % j
    # print(p2)
    # with open(p2, "wb")as f:
